...
const els = {
  ...
  premiumBadgeDot: document.getElementById('premiumBadgeDot'),
  ...
  cardPreviewTooltip: document.getElementById('cardPreviewTooltip'),
  cardPreviewTitle: document.getElementById('cardPreviewTitle'),
  cardPreviewMeta: document.getElementById('cardPreviewMeta'),
  cardPreviewDesc: document.getElementById('cardPreviewDesc'),
  ...
  activityWidgetBubble: document.getElementById('activityWidgetBubble'),
  activityWidget: document.getElementById('activityWidget'),
  activityWidgetPanel: document.getElementById('activityWidgetPanel'),
  activityWidgetBadge: document.getElementById('activityWidgetBadge'),
  activityFriendsBody: document.getElementById('activityFriendsBody'),
  activityUpdatesBody: document.getElementById('activityUpdatesBody'),
  activityUpdatesTabBadge: document.getElementById('activityUpdatesTabBadge'),
...
let friendsList = [];
// id -> { kind: 'reading'|'watching', title, meta } — активность друзей,
// приходит вместе с presence-событием (см. onOnlineEvent, case 'presence')
let friendsActivity = {};
// список найденных новых глав/серий (от library:updatesFound) — каждой
// записи присваивается свой id, чтобы можно было убрать конкретную из списка
let libraryUpdates = [];
let libraryUpdatesSeenIds = new Set();
...
els.premiumThemeToggleBtn.addEventListener('click', () => {
  const next = document.body.dataset.theme === 'premium' ? lastNonPremiumTheme : 'premium';
  applyTheme(next);
  window.hanko.saveSettings({ theme: next });
});
...
function showCardPreview(card, item, metaText) {
  const rect = card.getBoundingClientRect();
  els.cardPreviewTitle.textContent = item.title || '';
  els.cardPreviewMeta.hidden = !metaText;
  els.cardPreviewMeta.textContent = metaText || '';
  els.cardPreviewDesc.textContent = item.description || '';
  els.cardPreviewTooltip.hidden = false;
  els.cardPreviewTooltip.classList.remove('is-visible');

  const tooltipWidth = 260;
  let left = rect.right + 10;
  if (left + tooltipWidth > window.innerWidth - 12) left = rect.left - tooltipWidth - 10;
  left = Math.max(12, Math.min(left, window.innerWidth - tooltipWidth - 12));
  els.cardPreviewTooltip.style.left = `${left}px`;
  els.cardPreviewTooltip.style.top = `${rect.top}px`;

  requestAnimationFrame(() => {
    const h = els.cardPreviewTooltip.offsetHeight;
    let top = rect.top;
    if (top + h > window.innerHeight - 12) top = Math.max(12, window.innerHeight - h - 12);
    els.cardPreviewTooltip.style.top = `${top}px`;
    els.cardPreviewTooltip.classList.add('is-visible');
  });
}
...
function hideCardPreview() {
  els.cardPreviewTooltip.classList.remove('is-visible');
  setTimeout(() => {
    if (!els.cardPreviewTooltip.classList.contains('is-visible')) els.cardPreviewTooltip.hidden = true;
  }, 150);
}
...
function jikanMangaCard(item) {
  ...
  attachCardPreview(card, item, item.score ? `★ ${item.score.toFixed(1)}` : '');
  return card;
}
...
function mangaCard(item, { inLibrary }) {
  ...
  attachCardPreview(card, item, statusRu ? `${statusRu}${item.rating ? ` · ★ ${item.rating.toFixed(1)}` : ''}` : '');
  return card;
}
...
function animeLibraryCard(item, { showRemove = true } = {}) {
  ...
  card.innerHTML = `
    ...
    <div class="card-body">
      <p class="card-title">${escapeHtml(item.title)}</p>
      <p class="card-meta">${escapeHtml(statusRu)}</p>
      <div class="card-progress-track"><div class="card-progress-fill" id="progress-anime-${item.id}"></div></div>
    </div>
  ...
  // та же логика, что у mangaCard — досчитываем реальную долю просмотренных
  // серий, только по общему числу серий тайтла вместо числа глав
  if (hist && typeof hist.episodeIndex === 'number' && item.id.startsWith('al:')) {
    window.hanko.anilibriaEpisodes(item.id).then((episodes) => {
      if (!episodes || !episodes.length) return;
      const percent = Math.round(((hist.episodeIndex + 1) / episodes.length) * 100);
      const fill = card.querySelector(`#progress-anime-${CSS.escape(item.id)}`);
      if (fill) fill.style.width = `${Math.min(100, Math.max(2, percent))}%`;
    }).catch(() => {});
  }

  attachCardPreview(card, item, statusRu);
  return card;
}
...
function renderProfileBookmarks() {
  ...
  for (const item of animeLibrary.slice(0, BOOKMARKS_PREVIEW_LIMIT)) {
    els.profileAnimeGrid.appendChild(animeLibraryCard(item, { showRemove: false }));
  }
...
function openBookmarksModal(kind) {
  ...
  for (const item of items) {
    els.bookmarksModalGrid.appendChild(isManga ? mangaCard(item, { inLibrary: true }) : animeLibraryCard(item, { showRemove: false }));
  }
...
async function connectOnline() {
  ...
  renderOnlineStatus();
  if (onlineState.ready) {
    // раньше syncLibraryAndHistoryOnce() запускался ПОСЛЕ того, как полностью
    // отработают refreshIncoming/Outgoing/Friends/Presence (await Promise.all
    // блокировал его старт) — из-за этого окно, в котором на Аниме/Манге ещё
    // висят локальные (возможно устаревшие) данные, было длиннее, чем нужно.
    // Запускаем сразу параллельно — оба набора запросов друг другу не мешают.
    await Promise.all([refreshIncoming(), refreshOutgoing(), refreshFriends(), refreshPresence(), syncLibraryAndHistoryOnce()]);
...
async function syncLibraryAndHistoryOnce() {
  try {
    ...
    // раньше тут не было — если приложение открывалось сразу на вкладке
    // Манга/Аниме (settings.lastTab), там уже успевал отрисоваться "Продолжить"
    // из ЛОКАЛЬНОЙ (возможно устаревшей) истории ДО того, как долетал более
    // свежий вариант из облака (эта функция асинхронная, не блокирует старт
        // приложения специально). Без переотрисовки конкретно этих секций старые
    // тайтлы так и оставались висеть до ручного переключения вкладки туда-сюда
    renderMangaContinue();
    renderAnimeContinue();
    // раньше тут не было — закладки в Профиле (манга и аниме) вообще никогда
    // не перерисовывались после этого фонового синка, только при уходе со
    // страницы Профиль и возврате на неё
    renderProfileBookmarks();
...
function renderPremiumBlock() {
  ...
  if (!onlineState.ready) {
    els.premiumBadgeDot.hidden = true;
  ...
  if (onlineState.isPremium) {
    els.premiumBadgeDot.hidden = false;
...
// тихий короткий "тик" на мелкие действия (добавил в закладки, отметил главу/
// серию прочитанной) — та же техника, что и у playNotificationSound, просто
// тише, короче и без вторых нот, чтобы не отвлекать так же, как уведомление
...
window.hanko.onOnlineEvent(async (event) => {
  if (event.type === 'friend-request-incoming') {
    ...
    friendsActivity = event.activity || {};
...
// ---------------- стрик — сколько дней подряд заходил в приложение ----------------
...
async function updateStreak(settings) {
  ...
  let streak = settings.streakCount || 0;
  if (last !== today) {
    streak = last === dateKeyOffset(-1) ? streak + 1 : 1;
    window.hanko.saveSettings({ streakLastDate: today, streakCount: streak }).catch(() => {});
  }
...
// ---------------- виджет "Друзья и новинки" ----------------

function friendActivityLabel(activity) {
  if (!activity || !activity.kind) return null;
  const verb = activity.kind === 'watching' ? 'Смотрит' : 'Читает';
  return `${verb} «${activity.title}»${activity.meta ? ' · ' + activity.meta : ''}`;
}

function renderActivityWidgetFriends() {
  const body = els.activityFriendsBody;
  const online = friendsList.filter((f) => onlineFriendIds.has(f.friend_id));
  if (!online.length) {
    body.innerHTML = '<p class="empty-hint" style="padding:8px 4px;">Сейчас никого из друзей нет в сети.</p>';
    return;
  }
  // сначала те, у кого есть активность (реально что-то читают/смотрят), потом просто онлайн
  const sorted = online.slice().sort((a, b) => (friendsActivity[b.friend_id] ? 1 : 0) - (friendsActivity[a.friend_id] ? 1 : 0));
  body.innerHTML = '';
  for (const f of sorted) {
    const name = f.display_name || 'Без имени';
    const label = friendActivityLabel(friendsActivity[f.friend_id]);
    const row = document.createElement('div');
    row.className = 'activity-row';
    row.innerHTML = `
      <span class="activity-row-avatar">${avatarInnerHtml(name, f.avatar_url)}<span class="activity-row-avatar-dot"></span></span>
      <div class="activity-row-text">
        <div class="activity-row-name">${escapeHtml(name)}</div>
        <div class="activity-row-sub">${escapeHtml(label || 'В сети')}</div>
      </div>
    `;
...
function updateActivityWidgetBadge() {
  ...
  els.activityWidgetBadge.textContent = count > 9 ? '9+' : String(count);
}

function renderActivityWidgetUpdates() {
  const body = els.activityUpdatesBody;
  body.innerHTML = '';
  if (!libraryUpdates.length) {
    ...
    for (const u of libraryUpdates) {
      ...
      row.innerHTML = `
        ...
        <div class="activity-row-text">
          <div class="activity-row-name">${escapeHtml(u.title)}</div>
          <div class="activity-row-sub">${escapeHtml(kindWord)}</div>
        </div>
      `;
      row.addEventListener('click', () => {
        toggleActivityWidgetPanel(false);
        if (u.kind === 'manga') {
          showView('manga');
          openTitleModal({ id: u.id, title: u.title, coverUrl: u.coverUrl });
        } else {
          showView('anime');
          openAnimeTitleModal({ id: u.id, title: u.title, coverUrl: u.coverUrl });
        }
      });
      body.appendChild(row);
    }
...
document.addEventListener('click', (e) => {
  if (els.activityWidgetPanel.hidden) return;
  if (!els.activityWidget.contains(e.target)) toggleActivityWidgetPanel(false);
});
els.activityWidgetPanel.querySelectorAll('.activity-widget-tab').forEach((tab) => {
  tab.addEventListener('click', () => {
    els.activityWidgetPanel.querySelectorAll('.activity-widget-tab').forEach((t) => t.classList.remove('is-active'));
...
// новые главы/серии — прилетают пачкой раз в 20 минут из main.js
...
async function loadAnimeSource(item, ep, sourceIndex) {
  ...
  window.hanko.onlineSetActivity({ kind: 'watching', title: item.title, meta: `Серия ${ep.number}` }).catch(() => {});
