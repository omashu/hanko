...
// облегчённая версия того же сопоставления источников, что и в
// mangadex:chapters — но без дорогой перепроверки "хрупких" источников
// (просто берём максимум по числу глав). Используется только для фонового
// сравнения "не появилось ли новых глав" — там не нужна такая же точность,
// как при реальном открытии тайтла на чтение
async function quickMangaChapterCount(mangaId, title) {
  ...
  const [mdRuItems, rmRuItems, waRuItems, mbRuItems, ugRuItems] = await Promise.all([
  ...
  return Math.max(mdRuItems.length, rmRuItems.length, waRuItems.length, mbRuItems.length, ugRuItems.length);
}

// то же самое для аниме — только по AniLibria (без AnimeOn, у которого своих
// айди для сопоставления пока нет под рукой в лёгком виде); для уведомления
// "вышла новая серия" этого достаточно, полное сведение источников остаётся
// только при реальном открытии тайтла
async function quickAnimeEpisodeCount(releaseId) {
  if (!releaseId.startsWith(ANILIBRIA_PREFIX)) return 0;
  const aniId = releaseId.slice(ANILIBRIA_PREFIX.length);
  try {
    return (await aniEpisodes(aniId)).length;
  } catch {
    return 0;
  }
}

// проходится по всей библиотеке манги/аниме, сравнивает текущее число
// глав/серий с тем, что было запомнено в прошлый раз (item.knownCount),
// обновляет library.json/animeLibrary.json и возвращает список тайтлов, где
// число выросло. Первый раз, когда у тайтла ещё не было knownCount вообще
// (только что добавили в библиотеку) — просто запоминаем текущее число, не
// уведомляем: иначе при каждом добавлении в закладки сыпалось бы "нашли все
// N глав как новые"
async function checkLibraryUpdates() {
  ...
  for (const item of lib) {
    try {
      const count = await quickMangaChapterCount(item.id, item.title);
      if (!count) continue;
      if (typeof item.knownCount === 'number' && count > item.knownCount) {
        found.push({ kind: 'manga', id: item.id, title: item.title, coverUrl: item.coverUrl, prevCount: item.knownCount, newCount: count });
      }
      if (item.knownCount !== count) { item.knownCount = count; libChanged = true; }
    } catch { /* пропускаем один тайтл, не роняем всю проверку */ }
  }
  if (libChanged) await saveLibrary(lib);

  const animeLib = await loadAnimeLibrary();
  let animeLibChanged = false;
  for (const item of animeLib) {
    try {
      const count = await quickAnimeEpisodeCount(item.id);
      if (!count) continue;
      if (typeof item.knownCount === 'number' && count > item.knownCount) {
        found.push({ kind: 'anime', id: item.id, title: item.title, coverUrl: item.coverUrl, prevCount: item.knownCount, newCount: count });
      }
      if (item.knownCount !== count) { item.knownCount = count; animeLibChanged = true; }
    } catch { /* пропускаем один тайтл */ }
  }
  if (animeLibChanged) await saveAnimeLibrary(animeLib);

  return found;
}

function startLibraryUpdatesWatcher() {
  const run = async () => {
    try {
      const found = await checkLibraryUpdates();
      if (found.length && mainWindow) mainWindow.webContents.send('library:updatesFound', found);
    } catch { /* не критично, попробуем на следующем цикле */ }
  };
  setTimeout(run, 20000); // не сразу при старте — не мешаем остальной загрузке
  setInterval(run, 20 * 60 * 1000); // и дальше каждые 20 минут, пока приложение открыто
}
...
// что человек сейчас читает/смотрит — не пишется в базу вообще, просто едет
// внутри того же presence-трека, что и обычный признак "онлайн" (см. ниже);
// null, если сейчас ничего не открыто
let myActivity = null;
...
function setupPresence(myId) {
  ...
  presenceChannel
    .on('presence', { event: 'sync' }, () => {
      ...
      // у каждого ключа presenceState хранит массив треков (обычно один — но
      // теоретически может быть несколько вкладок/окон одного и того же id);
      // берём activity из самого свежего трека, если он там есть
      const activity = {};
      for (const [id, entries] of Object.entries(state)) {
        const withActivity = entries.find((e) => e.activity);
        if (withActivity) activity[id] = withActivity.activity;
      }
      sendOnlineEvent({ type: 'presence', onlineIds: Array.from(onlineFriendIds), activity });
    })
    .subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        await presenceChannel.track({ at: new Date().toISOString(), activity: myActivity });
      }
    });
}

// вызывается при открытии главы манги/серии аниме (см. IPC online:setActivity) —
// перетрекиваем presence с новым значением, чтобы друзья увидели обновление
// почти сразу (это тот же самый websocket-трек, что и раньше, просто с более
// богатым payload)
async function setMyActivity(activity) {
  myActivity = activity;
  if (presenceChannel) {
    try { await presenceChannel.track({ at: new Date().toISOString(), activity: myActivity }); } catch { /* не критично */ }
  }
}
...
ipcMain.handle('online:setActivity', (_e, activity) => setMyActivity(activity));
